...
## 配置文件规则  
  ...
  特殊值是配置文件的核心，按功能分为窗口配置类、窗口信息类、时间日期类、功能类四类，以下是完整说明：
  ...
  **格式**：`[start]{x坐标,y坐标,宽度,高度,透明度,鼠标穿透,任务栏隐藏,Alt+Tab隐藏,仅高于桌面,完全置顶,显示最小化按钮,显示最大化按钮,显示关闭按钮,显示标题栏,启动最大化,启动最小化,最小化到托盘,自动刷新置顶时间,窗口标题}`
  **参数说明**：
...
| 19       | 启动窗口标题      | 字符串 | 任意文本 |
  ...
  **参数说明**：  
  ...
  这些特殊值可以在任何地方使用，用于获取窗口的实时信息。

  | 特殊值 | 功能说明 | 示例输出 |
  |--------|----------|----------|
  | [winx] | 窗口左上角x坐标（实时获取） | 100 |
  | [winy] | 窗口左上角y坐标（实时获取） | 100 |
  | [windx] | 窗口宽度（实时获取） | 800 |
  | [windy] | 窗口高度（实时获取） | 600 |
...
| [ms]   | 当前毫秒（3位，补零）         | 001、999 |
  ...
  **功能**：在指定时间检查程序版本更新，与全局变量`verrr`比较版本号。
  **格式**：`[update]{time,delay,timeout,"URL"}`
  **参数说明**：
  - `time`：检查时间，格式为`hhmmss`，若为`-1`则仅在启动后检查一次；
  - `delay`：在指定时间后的延迟秒数；
  - `timeout`：请求网址的超时秒数；
  - `URL`：获取版本号的网址（该网址应返回纯文本版本号）。
    **功能逻辑**：
  - 检查成功且为最新版：弹出蓝色i图标提示框（仅确定按钮）；
  - 检查失败：弹出红色x图标提示框（仅确定按钮）；
  - 有新版本：弹出蓝色i图标提示框（取消和打开网址按钮）。
  ...
  **功能**：在指定日期时间播放本地WAV音乐文件。
  **格式**：`[timemusic]{date,time,lasting,cycle,"Path"}`
  **参数说明**：
  - `date`：指定日期，格式为`yyyymmdd`，若为`0`则表示每天；
  - `time`：指定时间，格式为`hhmmss`，若为`-1`则仅在启动后执行一次；
  - `lasting`：播放持续时间（秒），若为`-1`则播放完毕为止；
  - `cycle`：循环播放次数，若为`-1`则无限循环；
  - `Path`：音频文件路径，支持绝对路径和相对路径，必须为WAV格式。
    **功能逻辑**：
  - 到达指定时间后自动播放音乐；
  - 根据`lasting`设置自动停止播放；
  - 根据`cycle`设置循环播放次数。
  ...
  **功能**：在指定日期时间异步打开一个程序。
  **格式**：`[timeapp]{date,time,lasting,hide?,"Path"}`
  **参数说明**：
  - `date`：指定日期，格式为`yyyymmdd`，若为`0`则表示每天；
  - `time`：指定时间，格式为`hhmmss`，若为`-1`则仅在启动后执行一次；
  - `lasting`：程序运行持续时间（秒），若为`-1`则不强制结束；
  - `hide?`：是否隐藏启动的程序窗口，`0`表示不隐藏，`1`表示隐藏；
  - `Path`：程序路径，支持绝对路径和相对路径。
    **功能逻辑**：
  - 到达指定时间后自动启动程序；
  - 根据`hide?`设置决定是否隐藏程序窗口；
  - 根据`lasting`设置决定是否在指定时间后强制结束程序进程。
...
## 鼠标交互功能
  ...
  | 操作 | 功能 |
  |------|------|
  | Ctrl+左键单击窗口 | 打开配置文件(config.txt) |
  | 右键单击窗口 | 打开配置文件(config.txt) |
  | Alt+左键单击窗口 | 打开程序所在目录并默认选中程序文件 |
  | Shift+左键单击窗口 | 打开程序官网（网址为全局变量iqonliURL） |

  有事没事请加QQ群：743278470，你可以报告错误，寻求帮助